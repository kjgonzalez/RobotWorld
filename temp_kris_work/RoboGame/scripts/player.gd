...
todo fix wallslide behavior
todo fix double jump behavior
todo add hitboxes
'''
extends CharacterBody2D
signal p1_grenade_activated(pos)
signal p1_shoot_activated(pos,dir)
signal sig_buttonpress(action:String)

const SPEED = 300.0
const SPD_KNOCKBACK = 1000
...
func _on_btn_returntostart_pressed():
    sig_buttonpress.emit('to_start')

func _on_btn_reset_pressed():
    sig_buttonpress.emit('reset')



func _on_hitbox_body_entered(body):
    if('player' in body.name): return
    if('TileMap' in body.name): return
    print(body.name,'hit')
    take_damage(body)

func take_damage(body):
    ''' reduce hp and perform knockback of some kind '''
    var dmg = body.DAMAGE
    var knock_dir = sign((position - body.position).x)
    print('damage:',dmg,'kb_dir:',knock_dir)
    velocity.x +=knock_dir*SPD_KNOCKBACK
